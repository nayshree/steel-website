(function($) {
	...
	if($('.main-header').length){
		siteHeaderHeight = $('.main-header').outerHeight();
	}
	...
	function headerStyle() {
		if($('.main-header').length){
			...
			if (windowpos >= siteHeaderHeight) {
				...
				$('.page-wrapper').css('padding-top', siteHeaderHeight);
	...
	//Submenu Dropdown Toggle — delegated so it works on dynamically-loaded headers
	$(document).on('click', '.main-header .main-menu li.dropdown .dropdown-btn', function() {
	...
	// Open drawer on hamburger click — delegated so it works after $.load()
	$(document).on('click', '.main-menu .navbar-toggle', function(e) {
		e.preventDefault();
	...
	// Toggle sub-menus inside the drawer
	$(document).on('click', '.mobile-drawer-nav .has-drawer-sub > a', function(e) {
		e.preventDefault();
		var $li = $(this).parent();
		var $sub = $li.children('.mobile-drawer-sub');
		// Collapse any other open sub-menu
		$('.mobile-drawer-nav .has-drawer-sub').not($li)
			.removeClass('drawer-sub-open')
			.children('.mobile-drawer-sub').slideUp(200);
		$li.toggleClass('drawer-sub-open');
		$sub.slideToggle(200);
	});
